...
function Container({
  ...
  children: (opened: boolean, setOpened: (v: boolean) => void) => JSXElement;
}) {
    ...
    <Container>
      {(opened) => (
        <Collapse axis="y" in={opened}>
          <div style={{ "background-color": "gray" }}>{paragraph}</div>
        </Collapse>
      )}
    </Container>
  );
...
function SimpleCollapse(props: Partial<CollapseProps> & { buttonProps?: any }) {
  const [local, others] = splitProps(props, ["children", "buttonProps"]);
  const [opened, setOpened] = createSignal(false);

  return (
    <div style={{ margin: "15px" }}>
      <Button onClick={() => setOpened((o) => !o)} {...local.buttonProps} />
