...
describe("@mantine/core/Alert", () => {
  ...
  it("renders close button based on withCloseButton prop", () => {
    const { container: withCloseButton } = render(() => (
      <Alert {...defaultProps} withCloseButton />
    ));
    ...
    expect(
      withCloseButton.querySelector(".mantine-Alert-closeButton")
    ).toBeInTheDocument();
    expect(
      withoutCloseButton.querySelectorAll(".mantine-Alert-closeButton")
    ).toHaveLength(0);
  });
