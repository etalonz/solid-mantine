...
describe("@mantine/core/Slider/Marks", () => {
  it("renders correct marks labels", () => {
    ...
    expect(
      container.querySelectorAll(".mantine-Slider-markWrapper")
    ).toHaveLength(2);
  ...
  it("calls onChange with mark value when mark label is pressed", async () => {
    ...
    const { container } = render(() => (
      <Marks {...defaultProps} onChange={spy} />
    ));
    await userEvent.click(
      container.querySelectorAll(".mantine-Slider-markLabel")[0]
    );
    expect(spy).toHaveBeenLastCalledWith(50);
    await userEvent.click(
      container.querySelectorAll(".mantine-Slider-markLabel")[1]
    );
