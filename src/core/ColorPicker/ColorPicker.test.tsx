...
describe("@mantine/core/ColorPicker", () => {
  ...
  it("renders swatches list based on prop", () => {
    const { container: withSwatches } = render(() => (
      <ColorPicker swatches={swatches} />
    ));
    const { container: withoutSwatches } = render(() => <ColorPicker />);
    expect(
      withSwatches.querySelectorAll(".mantine-ColorPicker-swatches")
    ).toHaveLength(1);
    expect(
      withoutSwatches.querySelectorAll(".mantine-ColorPicker-swatches")
    ).toHaveLength(0);
  });
  ...
  it("renders AlphaSlider based on color format", () => {
    ...
    expect(rgba.querySelectorAll(".mantine-ColorPicker-slider")).toHaveLength(
      2
    );
  });
  ...
  it("renders picker based on withPicker prop", () => {
    ...
    const { container: withoutPicker } = render(() => (
      <ColorPicker withPicker={false} />
    ));
    expect(
      withPicker.querySelectorAll(".mantine-ColorPicker-saturation")
    ).toHaveLength(1);
    expect(
      withoutPicker.querySelectorAll(".mantine-ColorPicker-saturation")
    ).toHaveLength(0);
  });
