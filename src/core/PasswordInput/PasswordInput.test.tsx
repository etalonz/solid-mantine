...
describe("@mantine/core/PasswordInput", () => {
  ...
  it("sets toggle button tabIndex based on toggleTabIndex prop", () => {
    const { container: focusable } = render(() => (
      <PasswordInput toggleTabIndex={0} />
    ));
    const { container: notFocusable } = render(() => (
      <PasswordInput toggleTabIndex={-1} />
    ));
    expect(focusable.querySelector("button")).toHaveAttribute("tabindex", "0");
    expect(notFocusable.querySelector("button")).toHaveAttribute(
      "tabindex",
      "-1"
    );
  });
