...
const getChip = (index: number, type: "radio" | "checkbox") =>
  screen.getAllByRole(type)[index];
...
describe("@mantine/core/ChipGroup", () => {
  ...
  it("sets chip type based on multiple prop", () => {
    const { container: multiple } = render(() => (
      <ChipGroup multiple {...defaultProps} value={["1"]} />
    ));
    const { container: single } = render(() => (
      <ChipGroup multiple={false} {...defaultProps} />
    ));
  ...
  it("supports uncontrolled state when multiple prop is false", async () => {
    ...
    expect(
      screen
        .getAllByRole("radio")
        .every((chip) => chip.getAttribute("checked") === null)
    ).toBe(true);
  ...
  it("supports controlled state when multiple prop is true", async () => {
    const spy = vi.fn();
    render(() => (
      <ChipGroup {...defaultProps} multiple onChange={spy} value={["test-2"]} />
    ));
