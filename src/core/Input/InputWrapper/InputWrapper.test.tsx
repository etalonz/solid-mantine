...
const queries = {
  getLabel: (container: HTMLElement) =>
    container.querySelector(".mantine-InputWrapper-label"),
  getError: (container: HTMLElement) =>
    container.querySelector(".mantine-InputWrapper-error"),
...
describe("@mantine/core/InputWrapper", () => {
  ...
  it("renders correct error, description and label", () => {
    ...
    expect(queries.getDescription(container).textContent).toBe(
      "test-description"
    );
  ...
  it("does not render error if error prop is boolean", () => {
    const { container } = render(() => (
      <InputWrapper {...defaultProps} error />
    ));
  ...
  it("renders required asterisk with required prop is true", () => {
    const { container: required } = render(() => (
      <InputWrapper {...defaultProps} required />
    ));
    const { container: notRequired } = render(() => (
      <InputWrapper {...defaultProps} required={false} />
    ));
  ...
  it("spreads props to label, description and error", () => {
    ...
    expect(queries.getDescription(container)).toHaveAttribute(
  ...
  it("sets label element based on labelElement prop", () => {
    const { container: label } = render(() => (
      <InputWrapper {...defaultProps} labelElement="label" />
    ));
    const { container: div } = render(() => (
      <InputWrapper {...defaultProps} labelElement="div" />
    ));
