...
describe("@mantine/core/NumberInput", () => {
  ...
  it("does not render rightSection if input is disabled, variant is unstyled or controls are hidden", () => {
    ...
    const { container: disabled } = render(() => (
      <NumberInput {...defaultProps} disabled />
    ));
    const { container: controlsHidden } = render(() => (
      <NumberInput {...defaultProps} hideControls />
    ));
    const { container: unstyled } = render(() => (
      <NumberInput {...defaultProps} variant="unstyled" />
    ));
  ...
  it("increments and decrements value with controls", async () => {
    ...
    const { container } = render(() => (
      <NumberInput value={0} step={10} onChange={spy} />
    ));
  ...
  it("exposes increment/decrement handlers with handlersRef prop", () => {
    ...
    expect(typeof ref.decrement).toBe("function");
  ...
  it("sets input value with a given precision", async () => {
    const spy = vi.fn();
    render(() => (
      <NumberInput max={10} min={0} step={6} precision={2} onChange={spy} />
    ));
  ...
  it("supports removing trailing zeros with precision", async () => {
    const spy = vi.fn();
    render(() => (
      <NumberInput precision={8} removeTrailingZeros onChange={spy} />
    ));
  ...
  it("sets state to min if input is empty and is incremented/decremented", async () => {
    const spy = vi.fn();
    const { container } = render(() => (
      <NumberInput max={10} min={0} step={6} onChange={spy} />
    ));
    await clickIncrement(container);
    expectValue("0");
    expect(spy).toHaveBeenLastCalledWith(0);
