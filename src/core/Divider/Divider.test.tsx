import { render, screen, itSupportsSystemProps } from "testing";
...
describe("@mantine/core/Divider", () => {
  ...
  it("does not render label if label prop is not set or orientation is set to vertical", () => {
    const { container } = render(() => (
      <Divider label="test-label" orientation="vertical" />
    ));
    expect(container.querySelectorAll(".mantine-Divider-label")).toHaveLength(
      0
    );
  });
});
