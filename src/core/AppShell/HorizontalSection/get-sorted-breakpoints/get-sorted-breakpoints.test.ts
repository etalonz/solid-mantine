...
describe("@mantine/core/AppShell/get-sorted-breakpoints", () => {
  it("returns sorted breakpoints with theme values", () => {
    expect(
      getSortedBreakpoints({ lg: 500, sm: 400, xs: 300 }, DEFAULT_THEME)
    ).toStrictEqual([
  ...
  it("returns sorted breakpoints with number values", () => {
    expect(
      getSortedBreakpoints({ 1250: 500, 834: 400, 455: 300 }, DEFAULT_THEME)
    ).toStrictEqual([
      ["455", 300],
      ["834", 400],
      ["1250", 500],
  ...
  it("produces results that do not depend on object keys order", () => {
    ...
    expect(
      getSortedBreakpoints({ 834: 400, 1250: 500, 455: 300 }, DEFAULT_THEME)
    ).toStrictEqual(results);
    expect(
      getSortedBreakpoints({ 834: 400, 455: 300, 1250: 500 }, DEFAULT_THEME)
    ).toStrictEqual(results);
  });
});
