...
describe("@mantine/core/Select", () => {
  ...
  it("renders hidden input with current input value", () => {
    ...
    expect(container.querySelector('input[name="custom-select"]')).toHaveValue(
      "test-1"
    );
    expect(
      container.querySelector('input[name="custom-select"]')
    ).toHaveAttribute("form", "custom-form");
  ...
  it("opens dropdown when input is clicked (searchable={false})", async () => {
    render(() => (
      <Select {...defaultProps} initiallyOpened={false} searchable={false} />
    ));
  ...
  it("opens dropdown when input is clicked (searchable={true})", async () => {
    render(() => (
      <Select {...defaultProps} initiallyOpened={false} searchable />
    ));
    expect(screen.queryAllByRole("option")).toHaveLength(0);
