...
describe("@mantine/core/Select/SelectItems", () => {
  it("renders nothing found message if data array is empty", () => {
    const { container: withData } = render(() => (
      <SelectItems {...defaultProps} data={data} />
    ));
    ...
    expect(
      withData.querySelectorAll(".mantine-Select-nothingFound")
    ).toHaveLength(0);
    expect(
      withoutData.querySelector(".mantine-Select-nothingFound").textContent
    ).toBe("test-nothing");
  });
  ...
  it("renders correct custom items", () => {
    const { container } = render(() => (
      <SelectItems
        {...defaultProps}
        itemComponent={(props: any) => <span data-item {...props} />}
      />
    ));
    expect(screen.getAllByRole("option")).toHaveLength(
      defaultProps.data.length
  ...
  it("calls onItemHover when mouse enters item", async () => {
    const spy = vi.fn();
  ...
  it("registers refs of all items", () => {
    ...
    render(() => <SelectItems {...defaultProps} itemsRefs={refs.current} />);
    expect(Object.keys(refs.current)).toHaveLength(defaultProps.data.length);
    expect(
      Object.keys(refs.current).every(
        (ref) => refs.current[ref] instanceof HTMLDivElement
      )
    ).toBe(true);
  });
