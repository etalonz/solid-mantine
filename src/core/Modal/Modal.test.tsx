...
describe("@mantine/core/Modal", () => {
  ...
  it("allows to hide close button", () => {
    ...
    expect(withoutButton.querySelectorAll(".mantine-Modal-close")).toHaveLength(
      0
    );
  });
  ...
  it("does not render modal header if close button is hidden and title is not passed", () => {
    const { container: withHeader } = render(() => (
      <Modal {...defaultProps} title="With header" />
    ));
    const { container: withoutHeader } = render(() => (
      <Modal {...defaultProps} title={null} withCloseButton={false} />
    ));
    expect(withHeader.querySelectorAll(".mantine-Modal-header")).toHaveLength(
      1
    );
    expect(
      withoutHeader.querySelectorAll(".mantine-Modal-header")
    ).toHaveLength(0);
  });
