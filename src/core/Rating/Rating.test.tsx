import { checkAccessibility, itSupportsSystemProps, render } from "testing";
...
describe("@mantine/core/Rating", () => {
  checkAccessibility([
    () => <Rating defaultValue={3.5} fractions={2} aria-label="test-rating" />,
  ]);
  ...
  it("supports defaultValue", () => {
    ...
    const checkedInput: HTMLInputElement = container.querySelector(
  ...
  it("supports hover events", async () => {
    ...
    const { container } = render(() => (
      <Rating defaultValue={2} onHover={spy} />
    ));

    container.firstElementChild.getBoundingClientRect = vi
      .fn()
    ...
    const firstInput: HTMLInputElement = container.querySelector(
    ...
    const secondInput: HTMLInputElement = container.querySelector(
      'input[data-active="true"]'
    );
  ...
  it("supports uncontrolled state", async () => {
    const { container } = render(() => (
      <Rating name="click-test" defaultValue={2} />
    ));
    await userEvent.click(
      container.querySelector('input[name="click-test"][value="5"] + label')
    );
    ...
    await userEvent.click(
      container.querySelector('input[name="click-test"][value="4"]')
    );
