...
function DynamicChildren_() {
  const [active, setActive] = createSignal(1);
  const [count, setCount] = createSignal(3);

  const items = Array(count())
    .fill(0)
    .map((_, index) => () => <Stepper.Step label={`Step ${index + 1}`} />);
  ...
  return (
    <>
      ...
      <Group mt={40}>
        ...
        <Button onClick={() => setActive(Math.min(count(), active() + 1))}>
          Next
        </Button>
        <Button
          variant="outline"
...
const sizes = MANTINE_SIZES.map((size, index) => () => (
  <Wrapper size={size} mt={index !== 0 ? 60 : 0} />
));

const verticalSizes = MANTINE_SIZES.map((size, index) => () => (
  <Wrapper size={size} mt={index !== 0 ? 60 : 0} orientation="vertical" />
));
...
export const Sizes = () => <div style={{ padding: "40px" }}>{sizes}</div>;
