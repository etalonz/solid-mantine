...
describe("@mantine/core/Step", () => {
  ...
  it("renders icon when withIcon is true", () => {
    ...
    expect(withoutIcon.querySelectorAll(".mantine-Step-stepIcon")).toHaveLength(
      0
    );
  });
  ...
  it("renders step body if Step has label or description", () => {
    ...
    const { container: withDescription } = render(() => (
      <Step description="test-description" />
    ));
    const { container: withoutBody } = render(() => <Step />);

    expect(withoutBody.querySelectorAll(".mantine-Step-stepBody")).toHaveLength(
      0
    );
    expect(withLabel.querySelector(".mantine-Step-stepLabel").textContent).toBe(
      "test-label"
    );
    expect(
      withDescription.querySelector(".mantine-Step-stepDescription").textContent
